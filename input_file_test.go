...
func TestInputFileCompressed(t *testing.T) {
	...
	output := NewFileOutput(fmt.Sprintf("/tmp/%d_0.gz", rnd), config, stat)
	for i := 0; i < 1000; i++ {
	...
	output2 := NewFileOutput(fmt.Sprintf("/tmp/%d_1.gz", rnd), config, stat)
	for i := 0; i < 1000; i++ {
...
func CreateCaptureFile(requestGenerator *RequestGenerator) *CaptureFile {
	...
	config := &FileOutputConfig{flushInterval: time.Minute, append: true}
	stat := statistic.NewStatisticCollector()
	outputFile := NewFileOutput(f.Name(), config, stat)
